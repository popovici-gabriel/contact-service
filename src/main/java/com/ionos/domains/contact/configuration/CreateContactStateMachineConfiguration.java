...
public class CreateContactStateMachineConfiguration
	...
	public void configure(StateMachineConfigurationConfigurer<CreateContactState, CreateContactEvent> config)
		...
		config.withConfiguration().machineId("create-contact").autoStartup(true)
				.listener(new StateMachineListenerAdapter<>() {
					@Override
					public void stateChanged(State<CreateContactState, CreateContactEvent> from,
							State<CreateContactState, CreateContactEvent> to) {
	...
	private void contactRegistryInitiated(StateContext<CreateContactState, CreateContactEvent> context) {
		...
		jobServiceClient.addJob(operationService.getActiveOperation(operation.getCorrelationId()));
	...
	private void contactPersistenceInitiated(StateContext<CreateContactState, CreateContactEvent> context) {
		final var event = (Event) context.getMessageHeader(EVENT.getHeader());
		jobServiceClient.addJob(operationService.getActiveOperation(event.getOperationId()));
		updateState(event, context.getTarget().getId());
	}
